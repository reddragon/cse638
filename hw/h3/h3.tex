...
If $T_i$ is the threshold at an unbalanced node at level $i$, then
$T_{i+1}$ is the threshold at the next level, which is guaranteed to
be within balance. Therefore, the number of nodes that caused the
imaginary subtree at level $i$ to go out of balance are:
