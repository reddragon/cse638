...
know that we can hash $n$ elements in $O(n)$ space, while maintaining
$O(1)$ query time.

Now, instead of explicitly maintaining all $\sqrt{u}$ sub-widgets, and the summary widget, 
we maintain only the summary widget explicitly for a widget, and allocate sub-widget as required.
